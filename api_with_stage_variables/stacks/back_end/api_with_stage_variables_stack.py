...
# %%
        ...
        # Since Lambda Alias has to be provided to API GW Method Handler. Lets grab that function handler
        greeter_fn_prod_handler = _lambda.Function.from_function_arn(
            self,
            id="secureGreeterFnProdAlias",
            function_arn=greeter_fn_prod_alias.function_arn
        ...
        greeter_method_get = greeter.add_method(
            ...
            integration=_apigw.LambdaIntegration(
                handler=greeter_fn_prod_handler,
                proxy=True
            )
        )
